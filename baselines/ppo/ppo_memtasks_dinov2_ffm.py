...
from baselines.ppo.modules import DinoV2SimpleEncoder
from baselines.ppo.modules.recurrent_adapters import FFMRecurrentAdapter
...
class Args:
    ...
    """whether to save model into the `checkpoints/aaai_final/runs/{run_name}/{TIME}` folder"""
    evaluate: bool = False
    ...
    ffm_memory_size: int = 64
    """FFM trace size m (value used for the FFM baseline in the SHM repo's POPGym comparison)"""
    ffm_context_size: int = 4
    """FFM context size c (authors' POPGym baseline value)"""
    ffm_output_size: int = 512
    """FFM read-out width fed to the actor/critic heads (= our GRU baseline's hidden size)"""
    color_aug: bool = False
    """CRES analogue: concat per-patch centered mean-RGB to encoder features (enable on color-cue tasks)"""
    caps_lambda_t: float = 0.0
    """CAPS temporal action-smoothness weight (actor-head only); 0 = off. Identical penalty to SRB-TR-CAPS."""
    saliency_ckpt: str = "analysis/ebm/path_a_head_v3.pt"
    """path to Path A v3 frozen saliency head used by the DinoV2SaliencyEncoder"""
...
class Agent(nn.Module):
    def __init__(self, envs, sample_obs):
        super().__init__()
        # Perception-matched harness: same frozen DINOv2 encoder as all baselines;
        # CRES analogue toggled by --color-aug (color-cue tasks only).
        self.feature_net = DinoV2SimpleEncoder(sample_obs=sample_obs, color_aug=args.color_aug)
        latent_size = self.feature_net.out_features

        # Vendored FFM cell (authors' code + init, see modules/ffm_vendored.py)
        # behind the GRU state interface — no re-initialisation applied.
        self.gru = FFMRecurrentAdapter(
            input_size=latent_size,
            memory_size=args.ffm_memory_size,
            context_size=args.ffm_context_size,
            output_size=args.ffm_output_size,
        )
        head_in = self.gru.output_size

        print('#'*50)
        print(f'{latent_size=}')
        print(f'FFM m={args.ffm_memory_size} c={args.ffm_context_size} '
              f'state={self.gru.hidden_size} out={head_in} color_aug={args.color_aug}')
        print('#'*50)
        print('\n')


        self.critic = nn.Sequential(
            layer_init(nn.Linear(head_in, 512)),
    ...
    def get_states(self, x, gru_state, done):
        ...
        for h, d in zip(hidden, done):
            d = d.float()
            h_ = (1.0 - d).view(1, -1, 1) * gru_state
            h, gru_state = self.gru(h.unsqueeze(0), h_)
    ...
    def get_action(self, x, gru_state, done, deterministic=False):
        hidden, gru_state = self.get_states(x, gru_state, done)
        action_mean = self.actor_mean(hidden)
        if deterministic:
            return action_mean, gru_state
        action_logstd = self.actor_logstd.expand_as(action_mean)
        action_std = torch.exp(action_logstd)
        probs = Normal(action_mean, action_std)

        return probs.sample(), gru_state
    ...
    def get_action_and_value(self, x, gru_state, done, action=None):
        ...
        return action, probs.log_prob(action).sum(1), probs.entropy().sum(1), self.critic(hidden), gru_state
...
if __name__ == "__main__":
    ...
    SAVE_DIR = f'checkpoints/aaai_final/{MODE}/{args.reward_mode}/{args.env_id}'
    ...
    initial_gru_state = torch.zeros(
        agent.gru.num_layers, args.num_envs, agent.gru.hidden_size
    ).to(device)  # hidden state

    initial_gru_state_val = torch.zeros(
        agent.gru.num_layers, args.num_eval_envs, agent.gru.hidden_size
    ).to(device)  # hidden state

    # Store pre-step GRU states for correct flat minibatch updates
    gru_states = torch.zeros(
        (args.num_steps, args.num_envs, agent.gru.num_layers, agent.gru.hidden_size),
        device=device,
    )
    ...
    for iteration in tqdm(range(1, args.num_iterations + 1), total=args.num_iterations, desc="Training"):
        ...
        next_gru_state = initial_gru_state.clone()
        final_values = torch.zeros((args.num_steps, args.num_envs), device=device)
        agent.eval()
        if iteration % args.eval_freq == 1:
            print("Evaluating")
            next_gru_state_val = initial_gru_state_val.clone()
            eval_obs, _ = eval_envs.reset()
            eval_metrics = defaultdict(list)
            num_episodes = 0
            for _ in range(args.num_eval_steps):
                with torch.no_grad():
                    act_eval, next_gru_state_val = agent.get_action(eval_obs, next_gru_state_val, next_done_eval, deterministic=True)
        ...
        for step in range(0, args.num_steps):
            ...
            gru_states[step] = next_gru_state.permute(1, 0, 2)
        ...
        b_gru_states = gru_states.reshape(
            (-1, agent.gru.num_layers, agent.gru.hidden_size)
        )
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                mb_gru_states = b_gru_states[mb_inds].transpose(0, 1).contiguous()
                ...
                # CAPS temporal action-smoothness (actor-head only; same penalty def as SRB-TR-CAPS).
                # GRU update uses random flat minibatches with cached per-step states, so use the
                # global-flat-index neighbor (i -> i+num_envs = same env, t+1), exactly as SRB-TR-CAPS.
                caps_loss = pg_loss.new_zeros(())
                if args.caps_lambda_t > 0.0:
                    mb_t = torch.as_tensor(mb_inds, device=device, dtype=torch.long)
                    nxt = mb_t + args.num_envs
                    valid = (mb_t < (args.num_steps - 1) * args.num_envs)
                    valid = valid & (b_dones[nxt.clamp(max=args.batch_size - 1)] < 0.5)
                    if valid.any():
                        vt = mb_t[valid]; vn = nxt[valid]
                        with torch.no_grad():
                            h_t, _ = agent.get_states(b_obs[vt], b_gru_states[vt].transpose(0, 1).contiguous(), b_dones[vt])
                            h_n, _ = agent.get_states(b_obs[vn], b_gru_states[vn].transpose(0, 1).contiguous(), b_dones[vn])
                        a_t = agent.actor_mean(h_t); a_n = agent.actor_mean(h_n)   # grad only -> actor
                        caps_loss = args.caps_lambda_t * ((a_t - a_n) ** 2).sum(-1).mean()
