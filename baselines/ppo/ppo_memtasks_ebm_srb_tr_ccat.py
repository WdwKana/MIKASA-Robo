...
from baselines.ppo.modules import EBMSRBTRCCATMemoryModule as EBMMemoryModule
...
if __name__ == "__main__":
    ...
    d_buf  = agent.ebm.buffer.d      # CCAT: buffer stores [dino, color] = d_vit + 3
    L_buf  = agent.ebm.L
    p_dim  = agent.ebm.reader.W_Q.in_features - 128  # query_in = [proprio, curr(128)]

    cache_features   = torch.zeros(args.num_steps, args.num_envs, L_buf, d_buf, device=device)
    ...
    for iteration in tqdm(range(1, args.num_iterations + 1), total=args.num_iterations, desc="Training"):
        ...
        b_features   = cache_features.reshape(-1, L_buf, d_buf)
