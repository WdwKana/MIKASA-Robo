...
class Args:
    ...
    belief_deterministic: bool = False
    """[works only with include_belief=True] if toggled, use deterministic mean instead of random sampling for belief"""
...
class NatureCNN(nn.Module):
    def __init__(self, sample_obs, belief_model=None, belief_deterministic=False):
        """
        ...
        belief_deterministic: if True, use deterministic mean instead of random sampling for belief
        """
        super().__init__()
        self.belief_model = belief_model
        self.belief_deterministic = belief_deterministic
        if self.belief_model is not None:
            ...
            self.belief_encoder = nn.Sequential(
                nn.Linear(self.belief_sample_dim, 128),
                nn.ReLU(),
                nn.Linear(128, 128),
            )
        ...
        if self.belief_model is not None and 'rgb' in observations:
            if cached_belief_samples is None:
                with torch.no_grad():
                    ...
                    if self.belief_deterministic:
                        # DETERMINISTIC: use z=0 (prior mean) and decoder mean (no sampling)
                        # This tests if stochastic sampling hurts pretrain-finetune alignment
                        B = belief_context.shape[0]
                        z_zero = torch.zeros(B, self.belief_model.latent_dim, device=belief_context.device)
                        phi_mean, _ = self.belief_model.decoder_dist(z_zero, belief_context)
                        # Replicate the mean K times (all samples are identical)
                        used_samples = phi_mean.unsqueeze(1).expand(-1, self.num_belief_samples, -1)  # (B, K, D)
                    else:
                        # STOCHASTIC: Sample multiple latent particles conditioned on context.
...
class Agent(nn.Module):
    def __init__(self, envs, sample_obs, belief_model=None, belief_deterministic=False):
        super().__init__()
        self.belief_model = belief_model
        self.feature_net = NatureCNN(sample_obs=sample_obs, belief_model=belief_model, belief_deterministic=belief_deterministic)
...
if __name__ == "__main__":
    ...
    if args.include_belief and args.belief_path:
        ...
        if args.belief_deterministic:
            print(f"  [DETERMINISTIC MODE] Using z=0 and decoder mean (no random sampling)")
        else:
            print(f"  [STOCHASTIC MODE] Using random z and decoder sampling (default)")
    ...
    if MODE not in ['state', 'state_oracle']:
        agent = Agent(envs, sample_obs=next_obs, belief_model=belief_model, belief_deterministic=args.belief_deterministic).to(device)
    else:
