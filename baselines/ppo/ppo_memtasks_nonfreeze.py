...
class Args:
    ...
    finetune_belief: bool = True
    """if True, allow PPO gradients to update belief VAE (non-freeze)"""
...
class NatureCNN(nn.Module):
    def __init__(self, sample_obs, belief_model=None, finetune_belief=False):
        """
        ...
        self.finetune_belief = finetune_belief
        if self.belief_model is not None:
            if self.finetune_belief:
    ...
    def _sample_belief(self, context: torch.Tensor) -> torch.Tensor:
        if not self.finetune_belief:
            return self.belief_model.sample(context)
        # Reparameterized sampling to allow gradients to flow into belief model.
        batch_size = context.shape[0]
        z = torch.randn(batch_size, self.belief_model.latent_dim, device=context.device)
        mean, std = self.belief_model.decoder_dist(z, context)
    ...
    def forward(self, observations, belief_memory=None, cached_belief_samples=None, return_samples=False):
        ...
        if self.belief_model is not None and 'rgb' in observations:
            use_cached = cached_belief_samples is not None and not self.finetune_belief
            if use_cached:
                ...
                if self.finetune_belief:
                    # Detach memory to avoid long BPTT graphs across rollout steps.
                    belief_memory_in = belief_memory.detach() if belief_memory is not None else None
                    belief_context, new_belief_memory = self.belief_model(rgb_obs, belief_memory_in)
                    z_samples = [self._sample_belief(belief_context) for _ in range(self.num_belief_samples)]
                    used_samples = torch.stack(z_samples, dim=1)  # (B, K, D)
                    if new_belief_memory is not None:
                        new_belief_memory = new_belief_memory.detach()
                else:
                    with torch.no_grad():
...
class Agent(nn.Module):
    def __init__(self, envs, sample_obs, belief_model=None, finetune_belief=False):
        super().__init__()
        self.belief_model = belief_model
        self.feature_net = NatureCNN(
            sample_obs=sample_obs,
            belief_model=belief_model,
            finetune_belief=finetune_belief,
        )
...
if __name__ == "__main__":
    ...
    if MODE not in ['state', 'state_oracle']:
        agent = Agent(
            ...
            sample_obs=next_obs,
            belief_model=belief_model,
            finetune_belief=args.finetune_belief,
        ).to(device)
    else:
