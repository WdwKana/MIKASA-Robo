...
# A6 family: dual-cam wrapper + DinoV2 + saliency-head encoder
...
if __name__ == "__main__":
    ...
    # DUAL-CAMERA: base + hand concat'd channel-wise -> 6-ch RGB
    from baselines.ppo.utils_ebm import FlattenRGBDObservationWrapperMulti
    ...
    for iteration in tqdm(range(1, args.num_iterations + 1), total=args.num_iterations, desc="Training"):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.num_envs, envsperbatch):
                ...
                # Recompute hidden under no_grad, so gradient flows ONLY into actor_mean.
                # Minibatch is full per-env sequences ordered (t, env); reshape -> (T, k) for consecutive diffs.
                caps_loss = pg_loss.new_zeros(())
                if args.caps_lambda_t > 0.0:
                    with torch.no_grad():
                        hid_caps, _ = agent.get_states(
                            b_obs[mb_inds],
                            (next_lstm_state[0][:, mbenvinds], next_lstm_state[1][:, mbenvinds]),
                            b_dones[mb_inds])
                    a_det = agent.actor_mean(hid_caps)                       # (mb, act); grad only -> actor
                    a_seq = a_det.view(args.num_steps, envsperbatch, -1)
                    d_seq = b_dones[mb_inds].view(args.num_steps, envsperbatch)
                    diff = ((a_seq[1:] - a_seq[:-1]) ** 2).sum(-1)           # (T-1, k)
                    valid = (d_seq[1:] < 0.5).float()                        # drop pairs straddling a reset
                    caps_loss = args.caps_lambda_t * (diff * valid).sum() / valid.sum().clamp(min=1.0)
