"""
EBM-SRB-TR: Hierarchical Token Routing.

Resolves the limitation of SRB-MS (motion-suppress) which throws away
all moving-token information. The "moving tokens are noise" heuristic
breaks on Intercept tasks where the ball is the task-critical entity
AND constantly moves. Motion is not noise; it is information that should
flow to the appropriate memory subsystem.

We propose a structural decomposition based on the cortical principle of
processing time-scales at different hierarchical levels (predictive
coding, Rao & Ballard 1999; dynamic predictive coding RNN, Choi et al.
2017). Per-patch frame-to-frame change classifies tokens by their
predictive profile, and tokens are routed accordingly:

  HIGH-motion tokens (gripper, ball, environmental dynamics)
      → LSTM working memory  (online tracking of continuous state)
  LOW-motion + novel tokens (cube reveals, contact events)
      → Episodic K-V buffer  (sparse long-term event storage)

Both subsystems contribute to the actor via fuse; no information is
suppressed, only routed to its appropriate timescale-handler.

  ema_change[n] ← (1−α) · ema_change[n] + α · ‖p_t[n] − p_{t-1}[n]‖²

  buffer_write_score[n] = SRB[n] / (1 + λ · ema_change[n] / d_vit)
        ↑ same as SRB-MS — low-motion novel tokens go to buffer

  lstm_route_score[n] = ema_change[n]
        ↑ NEW — high-motion tokens go to LSTM input pool

  → top_K_buffer (low-motion novel)  → push to buffer
  → top_K_lstm   (high-motion)        → pooled → LSTM input

The architecture is parameter-free routing of frozen DINOv2 patches.
Working memory gets dynamics; episodic memory gets events.

API matches the SRB-MS module; PPO entry is a 1-line import swap.
"""
...
class EBMSRBTRMemoryModule(nn.Module):
    ...
    def step(self, rgb6: torch.Tensor, proprio: torch.Tensor, t: int) -> tuple[torch.Tensor, dict]:
        ...
        with torch.no_grad():
            ...
            # ── Routing (TR) ──────────────────────────────────────────────
            # Branch A: episodic buffer ← low-motion novel tokens (SRB-MS)
            ...
            cand_priority = topk_val_buf                  # priority for eviction

            # Branch B: LSTM working memory ← high-motion tokens (route here)
            # Use ema_change directly; the highest-motion patches are exactly
            # the things working memory should track (gripper, ball, etc.).
        ...
        # 4. curr_summary + pooled high-motion patches for LSTM context
        curr = self.curr_summary(torch.cat([cls_b, cls_h], dim=-1))
        # softmax-weighted pool over high-motion top-K (these go to working memory)
        pool_w = torch.softmax(topk_val_lstm, dim=-1).unsqueeze(-1)    # (B, K, 1)
        pooled = (pool_w * lstm_feats).sum(dim=1)                      # (B, d_vit)
