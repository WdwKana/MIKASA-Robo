"""
EBM-Hybrid-Clean (V3b) — V2 with GRU input cleaned up.

Motivation: V2's GRU input is `[pooled-top-K, curr_summary, proprio]`, which
duplicates information that the buffer cross-attention already supplies via
`retrieved`. The pooled-top-K is essentially "what the buffer currently
points at," so feeding it to the GRU makes the two paths redundant and
encourages the fuse layer to weight one at the expense of the other.

This variant feeds the GRU only `[proprio, curr_summary]` — i.e., the
agent's own state and the current frame summary. The GRU becomes a pure
trajectory integrator with no redundancy with the buffer path. Functional
roles split cleanly:
  - GRU      → continuous integration (velocity, motion, recent state)
  - Buffer   → episodic recall (what was seen earlier)
  - Reader   → cross-attention retrieval, query still static [proprio, curr]
  - Fuse     → combines retrieved + gru_hidden + proprio + curr

If V3b matches V2 on IMed AND ties/wins on RC9, the dilution penalty was
the GRU input, not the GRU itself.
"""
...
class EBMHybridCleanMemoryModule(nn.Module):
    def __init__(
        self,
        num_envs: int,
        proprio_dim: int = 25,
        ...
        # CLEAN GRU input: only proprio + curr (no buffer-derived pool).
    ...
    def step(self, rgb6: torch.Tensor, proprio: torch.Tensor, t: int) -> tuple[torch.Tensor, dict]:
        ...
        # CLEAN GRU step: no pooled buffer content.
