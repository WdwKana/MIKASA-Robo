"""
EBM-Belief-LSTM (V4-LSTM) — Belief-State Episodic Memory with LSTM.

Same dual-use principle as V4-GRU (see ebm_belief_gru.py header), but the
belief integrator is an LSTM. Hypothesis: the LSTM gates are better at
sustaining a useful belief over the 60-90 step episodes than a GRU,
particularly on Intercept tasks where ball-state needs persistent tracking.

Storage trick (same as V3a): (h, c) are concatenated along the last dim and
stored in a single (1, B, 2*H) buffer named `gru_state` so the PPO entry
caches/replays state identically to the GRU variants. The module splits and
re-joins the state inside `step` / `replay`.
"""
...
class EBMBeliefLSTMMemoryModule(nn.Module):
    def __init__(
        self,
        num_envs: int,
        proprio_dim: int = 25,
        ...
        # READER: query dim = H (belief drives retrieval).
        self.reader = MemoryReader(
            d_query=H, d_buffer=d_vit, d_proj=d_proj,
        )
        ...
        # (h, c) concatenated along last dim — single buffer for PPO cache.
