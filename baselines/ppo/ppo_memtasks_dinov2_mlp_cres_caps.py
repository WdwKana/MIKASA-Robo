    ...
    for iteration in tqdm(range(1, args.num_iterations + 1), total=args.num_iterations, desc="Training"):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # CAPS temporal action-smoothness (actor-head only; same penalty as SRB-TR-CAPS).
                # Memoryless MLP: random flat minibatch -> global-flat-index neighbor (i -> i+num_envs).
                ...
                if args.caps_lambda_t > 0.0:
                    ...
                    if valid.any():
                        vt, vn = mb_t[valid], nxt[valid]
                        with torch.no_grad():
                            ft = agent.feature_net(b_obs[vt])
                            fn = agent.feature_net(b_obs[vn])
                        a_t = agent.actor_mean(ft); a_n = agent.actor_mean(fn)   # grad only -> actor
