...
from baselines.ppo.modules.dinov2_poolcls_encoder import DinoV2PoolCLSEncoder as DinoV2SimpleEncoder
...
class Agent(nn.Module):
    def __init__(self, envs, sample_obs):
        super().__init__()
        # A6 family: replace NatureCNN with DinoV2 + frozen saliency-head encoder.
        self.feature_net = DinoV2SimpleEncoder(sample_obs=sample_obs, color_aug=True)
        latent_size = self.feature_net.out_features

        self.gru_hidden_size = args.gru_hidden_size
        self.gru = nn.GRU(latent_size, 
        ...
        print('#'*50)
        print(f'{latent_size=}')
        print(f'{self.gru.hidden_size=}')
        print('#'*50)
        print('\n')


        self.critic = nn.Sequential(
            layer_init(nn.Linear(self.gru_hidden_size, 512)),
