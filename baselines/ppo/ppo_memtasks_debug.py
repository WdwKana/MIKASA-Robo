...
class Args:
    ...
    eval_belief_ablation: bool = False
    """if True, run a second eval pass with belief_feat zeroed to measure impact"""
...
if __name__ == "__main__":
    ...
    for iteration in tqdm(range(1, args.num_iterations + 1), total=args.num_iterations, desc="Training"):
        ...
        if iteration % args.eval_freq == 1:
            print("Evaluating")
            def run_eval_pass(ablate_belief: bool, label: str):
                eval_obs, _ = eval_envs.reset()
                # Reset eval belief memory at start of evaluation
                eval_bm = None
                if belief_model is not None:
                    eval_bm = torch.zeros(args.num_eval_envs, belief_model.memory_size, device=device)
                eval_metrics_local = defaultdict(list)
                num_episodes_local = 0
                eval_step_counter = 0

                def eval_action_step(obs, bm):
                    # Custom path to optionally ablate belief features
                    if belief_model is None:
                        act, bm_new = agent.get_action(obs, deterministic=True)
                        return act, bm_new
                    feats, bm_new = agent.feature_net(obs, bm)
                    belief_dim = getattr(agent.feature_net, "belief_feat_dim", 0)
                    if ablate_belief and belief_dim > 0:
                        feats = feats.clone()
                        feats[:, -belief_dim:] = 0.0
                    act_mean = agent.actor_mean(feats)
                    return act_mean, bm_new

                for _ in range(args.num_eval_steps):
                    with torch.no_grad():
                        # ===== BELIEF DEBUG / VIZ (only on non-ablated, keeps TB清晰) =====
                        if (
                            (not ablate_belief)
                            and belief_model is not None
                            and logger is not None
                            and eval_step_counter < 50
                            and (eval_step_counter % 10 == 0)
                        ):
                            viz_step = global_step + eval_step_counter  # avoid TB step collision
                            bm_prev = eval_bm  # belief memory before update

                            # Belief memory magnitude
                            logger.add_scalar(
                                "belief_debug/belief_memory_norm",
                                bm_prev.norm(dim=1).mean(),
                                viz_step,
                            )

                            # Log an rgb frame (env0) if present
                            if "rgb" in eval_obs:
                                img0 = eval_obs["rgb"][0].detach().cpu().float()
                                if img0.max() > 1.0:
                                    img0 = img0 / 255.0
                                logger.writer.add_image(
                                    "belief_debug/eval_rgb_env0",
                                    img0.permute(2, 0, 1),
                                    viz_step,
                                )

                                # Sample phi from belief model to inspect uncertainty
                                ctx, _ = belief_model(eval_obs["rgb"], bm_prev)
                                phi_samples = torch.stack(
                                    [belief_model.sample(ctx) for _ in range(20)], dim=1
                                )  # (B, K, D)
                                phi_std = phi_samples.std(dim=1)  # (B, D)
                                logger.add_scalar(
                                    "belief_debug/phi_std_mean", phi_std.mean(), viz_step
                                )
                                logger.writer.add_histogram(
                                    "belief_debug/phi_std_env0",
                                    phi_std[0].detach().cpu(),
                                    viz_step,
                                )

                            # Check how much belief_feat affects actor/critic
                            belief_dim = getattr(agent.feature_net, "belief_feat_dim", 0)
                            if belief_dim > 0:
                                feats_dbg, _ = agent.feature_net(eval_obs, bm_prev)
                                belief_feat = feats_dbg[:, -belief_dim:]
                                logger.add_scalar(
                                    "belief_debug/belief_feat_norm",
                                    belief_feat.norm(dim=1).mean(),
                                    viz_step,
                                )
                                logger.writer.add_histogram(
                                    "belief_debug/belief_feat_env0",
                                    belief_feat[0].detach().cpu(),
                                    viz_step,
                                )

                                feats_abl = feats_dbg.clone()
                                feats_abl[:, -belief_dim:] = 0.0
                                a_with = agent.actor_mean(feats_dbg)
                                a_wo = agent.actor_mean(feats_abl)
                                v_with = agent.critic(feats_dbg)
                                v_wo = agent.critic(feats_abl)
                                logger.add_scalar(
                                    "belief_debug/action_ablation_l1",
                                    (a_with - a_wo).abs().mean(),
                                    viz_step,
                                )
                                logger.add_scalar(
                                    "belief_debug/value_ablation_abs",
                                    (v_with - v_wo).abs().mean(),
                                    viz_step,
                                )
                        # ===== END BELIEF DEBUG / VIZ =====

                        eval_action, eval_bm = eval_action_step(eval_obs, eval_bm)
                        ...
                        if belief_model is not None:
                            ...
                            if eval_done.any():
                                eval_bm[eval_done] = 0.0
                        if "final_info" in eval_infos:
                            mask = eval_infos["_final_info"]
                            num_episodes_local += mask.sum()
                            for k, v in eval_infos["final_info"]["episode"].items():
                                eval_metrics_local[k].append(v)
                    eval_step_counter += 1

                print(f"[{label}] Evaluated {args.num_eval_steps * args.num_eval_envs} steps resulting in {num_episodes_local} episodes")
                csv_row = {
                    "iteration": iteration,
                ...
                for k, v in eval_metrics_local.items():
                    mean = torch.stack(v).float().mean()
                    if logger is not None:
                        logger.add_scalar(f"{label}/{k}", mean, global_step)
                    print(f"{Fore.GREEN}{label} Metric: {k}{Style.RESET_ALL} | {Fore.CYAN}Mean: {mean:.4f}{Style.RESET_ALL}")
            ...
            # Run normal eval
            run_eval_pass(ablate_belief=False, label="eval")
            # Optional ablation eval
            if args.eval_belief_ablation and belief_model is not None:
                run_eval_pass(ablate_belief=True, label="eval_ablate_belief")
            if args.evaluate:
                break
