...
from baselines.ppo.modules.recurrent_adapters import SHMRecurrentAdapter
...
class Args:
    ...
    shm_mem_size: int = 72
    """SHM memory size H (authors' train_popgym.py default: --m 72); state is HxH per env"""
    shm_post_size: int = 1024
    """SHM read-out width fed to the actor/critic heads (authors' default: --post_size 1024)"""
...
class Agent(nn.Module):
    def __init__(self, envs, sample_obs):
        ...
        # Vendored SHM cell (authors' code + init incl. released-code quirks,
        # see modules/shm_vendored.py) behind the GRU state interface —
        # no re-initialisation applied.
        self.gru = SHMRecurrentAdapter(
            input_size=latent_size,
            mem_size=args.shm_mem_size,
            post_size=args.shm_post_size,
        )
        head_in = self.gru.output_size

        print('#'*50)
        print(f'{latent_size=}')
        print(f'SHM H={args.shm_mem_size} state={self.gru.hidden_size} '
              f'out={head_in} color_aug={args.color_aug}')
        print('#'*50)
        print('\n')
